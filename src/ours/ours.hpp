...
class fkco {
    ...
    bool greedy() {
        ...
        // inverted file
        std::vector<std::unordered_map<std::string, std::unordered_set<unsigned int>>> clusters(k);
        std::vector<int> center_dic(size);
        for (unsigned int i = 0; i < size; ++i) center_dic[i] = -1;
        ...
        for (unsigned int i = 1; i <= k; ++i) {
            ...
            for (unsigned int j = 0; j < size; ++j) {
                ...
                if (dist_min_array[j] > distance) {
                    dist_min_array[j] = distance;
                    
                    clusters[i-1][pset[j].group].insert(j);

                    // get dic
                    int prev = center_dic[j];
                    if (prev != -1) {
                        clusters[prev][pset[j].group].erase(j);
                        if (clusters[prev][pset[j].group].size() == 0) clusters[prev].erase(pset[j].group);
                    }
        ...
        for (unsigned int i = 0; i < center_set.size(); ++i) {

            auto it = clusters[i].begin();
            while (it != clusters[i].end()) {

                auto itr = it->second.begin();    
                std::uniform_int_distribution<> rnd_int_member(0, it->second.size() - 1);
                int rnd = rnd_int_member(mt);
                float distance = 0;
                while (rnd > 0) {
                    distance = compute_distance(&pset[center_set[i]], &pset[*itr]);
                    if (distance <= radius_guess) break;
                    ++itr;
                    --rnd;
                }
                group_in_center[i].insert({it->first, {distance, *itr}});
...
public:
    ...
    void output_file(bool flag) {
        ...
        f_name += "id(" + std::to_string(dataset_id) + ")_k(" + std::to_string(k) + ")_z(" + std::to_string(z) + ")_eps(" + std::to_string(eps) + ")_m(" + std::to_string(group_size) + ")_n(" + std::to_string(cardinality) + ")_ours.csv";
        std::ofstream file;
